...
LEGACY_TIMETABLE_RESET_VERSION = "universal-v1"
...
def get_connection():
    ...
    # Persistent browser login tokens.
    con.execute("""CREATE TABLE IF NOT EXISTS login_tokens (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        student_id INTEGER NOT NULL,
...
def create_login_token(student_id):
    token = secrets.token_urlsafe(48)
    now = now_india()
    expires = now + pd.Timedelta(days=365)
    con = get_connection()
    try:
        con.execute("DELETE FROM login_tokens WHERE student_id=?", (student_id,))
        con.execute(
            "INSERT INTO login_tokens(student_id, token_hash, created_at, expires_at) VALUES(?,?,?,?)",
            (student_id, _token_hash(token), now.isoformat(), expires.isoformat()),
        )
        con.commit()
    finally:
        con.close()
...
def login_from_token(token):
    ...
    con = get_connection()
    try:
        row = con.execute("""SELECT s.id, s.full_name, s.roll_number,
            COALESCE(s.course,''), COALESCE(s.division,''), COALESCE(s.academic_year,'')
            FROM login_tokens t JOIN students s ON s.id=t.student_id
            WHERE t.token_hash=? AND t.expires_at>?""",
            (_token_hash(str(token)), now_india().isoformat()),
        ).fetchone()
        if not row:
            return None
        return {"id": row[0], "full_name": row[1], "roll_number": row[2],
                "course": row[3], "division": row[4], "academic_year": row[5]}
    finally:
        con.close()


def delete_login_token(student_id, token=None):
    con = get_connection()
    try:
        if token:
            con.execute("DELETE FROM login_tokens WHERE student_id=? AND token_hash=?",
                        (student_id, _token_hash(str(token))))
        else:
            con.execute("DELETE FROM login_tokens WHERE student_id=?", (student_id,))
        con.commit()
    finally:
...
def clear_legacy_timetable_once():
    # Clears only saved weekly timetable rows from the old version.
    # Student accounts and attendance history are preserved.
    ...
    row = con.execute(
        "SELECT value FROM app_meta WHERE key='legacy_timetable_reset'"
    ).fetchone()
    if not row or row[0] != LEGACY_TIMETABLE_RESET_VERSION:
        con.execute("DELETE FROM timetable")
        con.execute(
            "INSERT OR REPLACE INTO app_meta(key,value) VALUES('legacy_timetable_reset',?)",
            (LEGACY_TIMETABLE_RESET_VERSION,)
        )
        con.commit()
    con.close()
...
# Restore login from the browser cookie after Chrome/browser is reopened.
cookie_controller = CookieController() if CookieController is not None else None
if not st.session_state.logged_in and cookie_controller is not None:
    try:
        saved_token = cookie_controller.get("attendance_login_token")
        remembered = login_from_token(saved_token) if saved_token else None
        if remembered:
            st.session_state.logged_in=True; st.session_state.student_id=remembered["id"]; st.session_state.student_name=remembered["full_name"]; st.session_state.roll_number=remembered["roll_number"]; st.session_state.course=remembered["course"]; st.session_state.division=remembered["division"]; st.session_state.academic_year=remembered["academic_year"]
    except Exception:
        pass
...
if not st.session_state.logged_in:
    ...
    with login_tab:
        ...
        if st.button("🔐 Login",type="primary",width="stretch"):
            ...
            if student:
                st.session_state.logged_in=True; st.session_state.student_id=student["id"]; st.session_state.student_name=student["full_name"]; st.session_state.roll_number=student["roll_number"]; st.session_state.course=student["course"]; st.session_state.division=student["division"]; st.session_state.academic_year=student["academic_year"]
                if cookie_controller is not None:
                    try:
                        token=create_login_token(student["id"])
                        cookie_controller.set("attendance_login_token", token, max_age=365*24*60*60)
                    except Exception:
                        pass
                st.rerun()
...
with st.sidebar:
    ...
    if st.button("🚪 Logout",width="stretch"):
        try:
            delete_login_token(student_id, cookie_controller.get("attendance_login_token") if cookie_controller is not None else None)
            if cookie_controller is not None:
                cookie_controller.remove("attendance_login_token")
        except Exception:
            pass
...
with setup_tab:
    ...
    st.info("Nothing is hard-coded to any course, division, academic year, subject, or weekday. Every student creates their own timetable.")
    with st.form("profile"):
        course=st.text_input("Course / Class",value=st.session_state.course,placeholder="B.Com / BCA / BBA / Class 12 / Any Class")
        division=st.text_input("Division / Section",value=st.session_state.division,placeholder="A / B / Any Section")
    ...
    st.caption("Only timetable images are imported. The app reads images with OCR, then gives you an editable preview. OCR is best-effort, so always check the preview.")
    ...
    st.divider()
    st.subheader("🗑️ Reset My Weekly Timetable")
    st.caption("If an old timetable from an earlier version is showing, clear your saved weekly timetable here. Attendance history is not deleted.")
    if st.button("🗑️ Clear My Weekly Timetable"):
        con=get_connection()
        con.execute("DELETE FROM timetable WHERE student_id=?",(student_id,))
        con.commit(); con.close()
        st.success("Weekly timetable cleared. Upload your own timetable images now.")
        st.rerun()
