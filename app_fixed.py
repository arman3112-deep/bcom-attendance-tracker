...
with dashboard_tab:
    ...
    if data.empty:
        ...
        for r in report:
            ...
            planner.append({"Subject": r["Subject"], "Current %": r["Attendance %"], "Present": r["Present"], "Total Conducted": r["Total"], "Need to Reach 75%": r["Need for 75%"], "Can Miss at 75%": r["Can Miss"], "Status": text})
        st.dataframe(pd.DataFrame(planner), width="stretch", hide_index=True)
        ...
        for r in report:
            ...
            if r["Attendance %"] < 75:
                any_low = True
                st.error(f"🔴 {r['Subject']}: {r['Attendance %']}% — attend the next {r['Need for 75%']} class(es) to reach 75%.")
            else:
                st.success(f"🟢 {r['Subject']}: {r['Attendance %']}% — you can miss about {r['Can Miss']} class(es) and remain at or above 75%.")
        if not any_low:
            st.success("🎉 All subjects with recorded classes are currently at or above 75%.")
...
with history_tab:
    ...
    if data.empty:
        ...
        history = data.copy()
        history["attendance_date"] = pd.to_datetime(history["attendance_date"]).dt.strftime("%d-%m-%Y")
        history.columns = ["Date", "Subject", "Status"]
        st.dataframe(history, width="stretch", hide_index=True)
        st.download_button("⬇️ Download Attendance CSV", history.to_csv(index=False).encode(), "attendance.csv", "text/csv", width="stretch")
